...
\part{c}
We can do this numerically using the bisection method.
Let $f(x) = x^{3} + 2x - 5$

Initialize $x_{min} = 1$ and $x_{max} = 2$.
$f(x_{min}) = x_{min}^{3} + 2x_{min} - 5 = 1 + 2 - 5 = -2$
$f(x_{max}) = x_{max}^{3} + 2x_{max} - 5 = 8 + 4 - 5 = 7$
