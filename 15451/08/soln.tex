...
Increment i and compute the hashes again. Note that this takes constant time since s[0:i] -> s[0:i+1] and s[i:2i] -> s[i+1:2i+2] so to compute the hash we only have to do a constant time adjustment to the previously computed hash.

Repeat until $i > t/2$. Return the max $i$ for which the two hash computed at the $ith$ iteration are equal.

\textbf{Proof that Pr[false positive] \lessthan 1/2}:
