function [feat_names, feat_vals, feat_slf] = ...
    ml_features(procimage, dnaimage, imagemask, dnaimagemask, featsets, scale, radius, ...
		nonobjimg, har_pixsize, har_intbins)

% [FEAT_NAMES, FEAT_VALS, FEAT_SLF] = ML_FEATURES( PROCIMAGE, DNAIMAGE,
% IMAGEMASK, FEATSETS, SCALE, RADIUS, NONOBJIMG, HAR_PIXSIZE, HAR_INTBINS)
%
...
if max(procimage(:))==0,
    ...
    for i = 1 : length( featsets)
        switch featsets{i}
            ...
            case 'hullsize'
                imagehull = ml_imgconvhull(imagemask);
                [hullnames, hullvalues, hullslf] = ml_hullfeatures(procimage, imagehull,'hullsize');
                feat_slf = [feat_slf hullslf];
                feat_names = [feat_names hullnames];
                feat_vals = [feat_vals hullvalues];
            case 'hullsizedna'
                % Find the convex hull
                dnahull = ml_imgconvhull(dnaimagemask);
                % Calc features
                [hullnames, hullvalues, hullslf] = ml_hullfeatures(dnaimage, dnahull,'dnahullsize');
		for i=1:numel(hullnames),
		    feat_slf = [feat_slf cellstr(['DNA:' hullslf{i}])];
		    feat_names = [feat_names cellstr(['DNA:' hullnames{i}])];
		end
                feat_vals = [feat_vals hullvalues];
            case 'alpha'
	    ...
	    case 'tas'
		% Calc features
		[tasnames, tasvalues, tasslf] = ml_tas(procimage);
		feat_slf = [feat_slf tasslf];
		feat_names = [feat_names tasnames];
		feat_vals = [feat_vals tasvalues];
            case 'pftas'
                [tasnames, tasvalues, tasslf] = ml_tas(procimage,0);
                feat_slf = [feat_slf tasslf];
                feat_names = [feat_names tasnames];
                feat_vals = [feat_vals tasvalues];
	    case 'tas'
                [tasnames, tasvalues, tasslf] = ml_tas(procimage,1);
                feat_slf = [feat_slf tasslf];
                feat_names = [feat_names tasnames];
                feat_vals = [feat_vals tasvalues];
            otherwise
                error(['Unrecognized feature set identifier in FEATSETS: "' featsets{i} '"']);
        end
    end
