...
%  FEATSETNAME, SCALE, RADIUS, BGSUB, THRESHMETH, HAR_PIXSIZE, HAR_INTBINS)
%
...
%             'tas'           - Threshold adjacency statistics (18)
%             'mcell'         - 26 field level (multiple cell) features
%             'mcell_mor'     - 13 morphorlogical field level features (subset of 'mcell')
%             'mcell_noskel'  - 21 field level features (exclude skeleton features from 'mcell')  
%             'mcell_all'     - 86 field level features ('mcell' plus 60 Gabor Wavelet features)
%             'best37'          - SLF5 (37)
%             'best34'          - Like SLF5 without DNA (34)
%             'all84'           - SLF4 (84)
%             'all114'          - SLF + wavelet features (114)
...
%             'imgdna'          - SLF2 without hull and edge features (14)
%             'alpha'           - alpha hull features (110)
%             'pftas'           - Threshold adjacency statistics, parameter free (18)
%             'tas'             - Threshold adjacency statistics, original version (18)
%             'mcell'           - 26 field level (multiple cell) features
%             'mcell_mor'       - 13 morphorlogical field level features (subset of 'mcell')
%             'mcell_noskel'    - 21 field level features (exclude skeleton features from 'mcell')  
%             'mcell_all'       - 86 field level features ('mcell' plus 60 Gabor Wavelet features)
%             'hullsize'        - 10 features for resolution determination
%             'hullsizednaonly' - 10 features for resolution determination, calculated on DNA channel only
%             'hulldna'         - Combination of 'hullsize' and 'hullsizednaonly' (20)
%
...
if( nargin <= MaxArgs-4)
    bgsub = 'lowcommon';
    %change the default bgsub method to lowcommon
    %bgsub = 'yesbgsub';
end
...
if iscell(featsetname)
	...
	subset_index = [1:10];
    case 'hullsize+dna'
	subsetlist = {'hullsize','hullsizedna'};
	use_dna = 1;
	subset_index = [1:20];
    case 'hullsizednaonly'
	subsetlist = {'hullsize'};
	image=dnaimage;
	dnaimage=[];
	use_dna = 0;
...
if( use_dna)
    ...
    [procdnaimage, dna_maskimage] = ml_preprocess( dnaimage, ...
        cropimage, way, bgsub, threshmeth);
    if( isempty(procdnaimage))
	disp('Giving up on feature calculation (image is empty)');
        features = zeros(1,length(subset_index))+NaN;
        slfnames = [];
        names = [];
        return
...
% Calculate features
[n,f,s] = ml_features( procimage, procdnaimage, prot_maskimage, dna_maskimage, ...
    subsetlist, scale, radius, nonobjimg, har_pixsize, har_intbins);

%t+
if isempty(f)
    disp('Feature vector was empty')
    features = zeros(1,length(subset_index))+NaN;
    names = {};
    slfnames = {};
else
