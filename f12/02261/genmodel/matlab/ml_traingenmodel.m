...
combfeats = ml_combccfeats(cellcodes);
...
%Cell shape model
model.cellShapeModel = ml_trainshapemodel(cellcodes(smallCellIndices), ...
...
%Protein model
% model.proteinModel.name = 'obj';
% model.proteinModel.objectModel.name = 'gauss';
% model.proteinModel.objectModel.covartype = ...
%     param.ml_objgaussmix.gmm.covartype;
% f = struct('funname','sqrt');
%Object model
% model.proteinModel.objectModel.stat  = ...
%     tz_trainlk(ml_evalfun(latents,f),struct('method','exp'));
% model.proteinModel.objectModel.stat.transform  = f;
...
model.proteinModel.positionModel = ml_initparam( ...
    ...
    struct('funname','ml_mappos','param',struct('order',2,'scale',1))));
