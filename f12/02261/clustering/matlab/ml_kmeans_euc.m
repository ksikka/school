...
while (~done)
	%keyboard
	clustera = zeros(n,1); % cluster assignments
    %no_clust = zeros(1, k);
	for m = 1 : n
		index = 1;
    ...
    sume=0;
	for l = 1 :k,
	    idxs=find(clustera == l);
	    if idxs,
            center(l,:) = mean(feat(idxs,:),1);
            for m = 1 : length(idxs),
                sume = sume + xc_squaresum2(feat(idxs(m),:)-center(l,:));
            end
	    end
	end
	if (~exist('prev', 'var'))
...
% Build cluster for output:
for l = 1 : k,
    cluster{l}=find(clustera == l);
end
...
% FUNCTION X = XC_SQUARESUM2(MTX)
% Calculate the squaresum of each column of the vector.  If
