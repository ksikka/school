function threshold = ml_kapurthreshold(img)  
% threshold = ml_kapurthreshold(img)
%
% Returns a threshold according to Kapur's method.
...
%
    ...
    while cumhisto(T) < cumhisto(end),
        value = ml_entropy(histo(1:T)/cumhisto(T)) + ml_entropy(histo(T+1:end)/(1-cumhisto(T)));
        if value > Best,
            threshold=T;
            Best = value;
        end
    ...
    disp(['Result: ' num2str(threshold)])
end

function H = ml_entropy(P),
    P(P==0)=1;
    H=-sum(P.*log(P));
end
