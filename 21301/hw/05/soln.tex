...
To maximize the degree of the smallest $S_i$, we consider the graph with the maximum possible number of edges such that $|E| = c_{3}n\sqrt{n}$, and we distribute the edges evenly accross vertexes in $B$, because giving any vertex more edges than another gives the other vertex a smaller degree.
In the above scenario, each vertex in $B$ would have degree $\frac{c_{3}n\sqrt{n}}{n} = c_{3}\sqrt{n}$. Thus we've shown that the maximum degree of the minimum degree vertex is $c_{3}\sqrt{n}$.

In any scenario such as the one given in the question, there will always exist an $S_i$ such that $d(S_i) = |S_i| \leq C \sqrt{n}$.
